...
import { Layout, Zap, Shield, CheckCircle } from "lucide-react";
    ...
    <div className="min-h-screen flex w-full bg-gray-50 relative">
      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-600 to-indigo-900 text-white p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
          ...
          <div
            className="absolute bottom-10 right-10 w-96 h-96 bg-purple-500 rounded-full mix-blend-overlay filter blur-3xl animate-pulse"
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-8 max-w-lg">
            ...
            <p className="text-lg text-blue-100">{t("auth.banner.subtitle")}</p>

            <div className="space-y-4 mt-8">
              <FeatureRow
                icon={Layout}
                text={t("auth.banner.features.templates")}
                delay="0"
              />
              <FeatureRow
                icon={Zap}
                text={t("auth.banner.features.export")}
                delay="100"
              />
              <FeatureRow
                icon={Shield}
                text={t("auth.banner.features.secure")}
                delay="200"
              />
              <FeatureRow
                icon={CheckCircle}
                text={t("auth.banner.features.ats")}
                delay="300"
              />
            </div>
          </div>
        ...
        <div className="relative z-10 text-sm text-blue-200">
          {t("auth.banner.copyright", { year: new Date().getFullYear() })}
        </div>
      </div>
...
const FeatureRow: React.FC<FeatureRowProps> = ({ icon: Icon, text, delay }) => (
  <div
    ...
    style={{
      animation: `slideIn 0.5s ease-out forwards ${delay}ms`,
      opacity: 0,
    }}
  >
