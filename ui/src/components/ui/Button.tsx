...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    variant?: 'primary' | 'danger' | 'ghost' | 'text' | 'toggle' | 'custom';
    color?: 'blue' | 'green' | 'purple' | 'orange' | 'pink' | 'teal' | 'indigo' | 'red';
...
export const Button: React.FC<ButtonProps> = ({
    ...
    const baseStyles = "inline-flex items-center justify-center rounded-lg transition-colors font-medium focus:outline-none focus:ring-2 focus:ring-offset-2";
    ...
    const variants: Record<string, string> = {
        primary: `${colorVariants[color] || colorVariants.blue} text-white py-2 px-4 shadow-sm`,
        danger: "bg-red-500 hover:bg-red-600 text-white py-2 px-4 shadow-sm focus:ring-red-500",
        ghost: "text-gray-400 hover:text-red-500 p-1 bg-transparent hover:bg-transparent focus:ring-0",
        text: `${textColors[color] || textColors.blue} text-sm bg-transparent hover:bg-transparent focus:ring-0`,
        toggle: isActive
    ...
    const variantClasses = variant === 'toggle'
        ? variants.toggle
        : (variants.hasOwnProperty(variant) ? variants[variant] : variants.primary);
