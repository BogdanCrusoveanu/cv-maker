...
export const useRenameCv = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ id, title }: RenameCvPayload) => {
      await api.patch(`/cv/${id}/title`, { title });
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["cvs"] });
      queryClient.invalidateQueries({
        queryKey: ["cv", variables.id.toString()],
      });
    },
  });
};
